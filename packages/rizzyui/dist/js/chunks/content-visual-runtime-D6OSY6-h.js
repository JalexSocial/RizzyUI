...
//#region src/js/lib/components/rzCarousel.js
function rzCarousel() {
	...
	return {
		...
		*/
		init() {
			...
			const self = this;
			if (assetsToLoad.length > 0 && typeof rizzyRequire === "function") rizzyRequire(assetsToLoad, {
				/**
				* Executes the `success` operation.
				* @returns {any} Returns the result of `success` when applicable.
				*/
				success() {
					if (window.EmblaCarousel) self.initializeEmbla(options, pluginsConfig);
					else console.error("[rzCarousel] EmblaCarousel not found on window after loading assets.");
				},
				/**
			...
			else if (window.EmblaCarousel) this.initializeEmbla(options, pluginsConfig);
			else console.error("[rzCarousel] EmblaCarousel not found and no assets specified for loading.");
		},
		/**
...
//#endregion
//#region src/js/lib/components/rzChart.js
function rzChart() {
	return {
		chartInstance: null,
		...
		resolveColorValues(config) {
			const walk = (value) => {
				...
				for (const key of Object.keys(value)) {
					...
					if (currentValue && typeof currentValue === "object") walk(currentValue);
				}
				return value;
			};
			walk(config);
		...
		_resolveColor(color, el = document.documentElement) {
			...
			const [varName, fallback] = value.slice(4, -1).trim().split(",").map((part) => part.trim());
			if (!varName) return value;
			return getComputedStyle(el).getPropertyValue(varName).trim() || fallback || value;
		},
		resolveCallbacks(config) {
			...
			const resolveStringFn = (fnName) => {
				...
				for (const part of parts) {
					if (context[part] === void 0) return fnName;
			...
			if (config.options.plugins?.tooltip?.callbacks) {
				...
				for (const key of Object.keys(callbacks)) callbacks[key] = resolveStringFn(callbacks[key]);
			}
			if (config.options.scales) for (const scaleKey of Object.keys(config.options.scales)) {
				const scale = config.options.scales[scaleKey];
				if (scale.ticks && scale.ticks.callback) scale.ticks.callback = resolveStringFn(scale.ticks.callback);
			}
		},
		destroy() {
...
//#endregion
//#region src/js/lib/components/rzHighlighter.js
	...
	return {
		...
		observeViewport() {
			...
			this.intersectionObserver = new IntersectionObserver((entries) => {
				if (!entries.some((entry) => entry.isIntersecting) || this.hasShown) return;
				this.showAnnotation();
		...
		showAnnotation() {
			...
			if (!target || typeof annotate !== "function") return;
			if (!this.annotation) this.annotation = annotate(target, {
		...
		observeResize() {
			const target = this.$refs.target;
			if (!target || typeof ResizeObserver === "undefined" || this.resizeObserver) return;
		...
		refreshAnnotation() {
			if (!this.hasShown || !this.annotation) return;
...
//#endregion
//#region src/js/lib/components/rzNumberTicker.js
function rzNumberTicker() {
	return {
		...
		tick(timestamp) {
			if (this.animationStartTimestamp === 0) this.animationStartTimestamp = timestamp;
		...
		formatNumber(value) {
			...
			try {
				if (this.culture) return format(this.culture);
			} catch (_) {}
			try {
				if (navigator.language) return format(navigator.language);
			} catch (_) {}
			return format("en-US");
		},
		setDisplay(value) {
			if (this.$refs.value) this.$refs.value.textContent = this.formatNumber(value);
		},
		complete() {
		...
		clampToDirection(value) {
			if (this.direction === "down") return Math.max(this.destinationValue, value);
...
//#endregion
//#region src/js/lib/components/rzShineBorder.js
function rzShineBorder() {
	return {
		...
		init() {
			...
			const gradientColors = shineColors.length > 0 ? shineColors.join(",") : shineColor.length > 0 ? shineColor : ["var(--color-border)", "color-mix(in oklab, var(--color-border) 75%, transparent)"].join(",");
			this.computedStyle = [
...
//#endregion
//#region src/js/lib/components/rzTypingAnimation.js
function rzTypingAnimation() {
	return {
		...
		resolveWords() {
			...
			if (explicitWords.length > 0) this.words = explicitWords;
...
//#endregion
export { rzCarousel, rzChart, rzHighlighter, rzNumberTicker, rzShineBorder, rzTypingAnimation };

//# sourceMappingURL=content-visual-runtime-D6OSY6-h.js.map