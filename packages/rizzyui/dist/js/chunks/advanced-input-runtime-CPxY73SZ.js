import { n as rizzyRequire } from "./bootstrap-D7iIajoc.js";
//#region src/js/lib/components/rzCombobox.js
function rzCombobox() {
	return {
		tomSelect: null,
		value: "",
		...
		*/
		init() {
			...
			if (assets.length > 0 && typeof rizzyRequire === "function") rizzyRequire(assets, {
				success: () => this.initTomSelect(),
				error: (err) => console.error("RzCombobox: Failed to load assets.", err)
			}, nonce);
			else if (window.TomSelect) this.initTomSelect();
		},
		/**
		...
		*/
		initTomSelect() {
			...
			const createAlpineRow = (templateRef, data) => {
				...
				if (typeof parsedItem === "string") try {
				...
				if (Alpine && typeof window.Alpine.addScopeToNode === "function") window.Alpine.addScopeToNode(div, scope);
				else div._x_dataStack = [scope];
...
//#endregion
//#region src/js/lib/components/rzFileInput.js
function rzFileInput() {
	return {
		...
		trigger() {
			...
			if (!input || input.disabled || this.isDisabled()) return;
			input.click();
		},
		handleFileChange() {
			this.syncFromInput({
		...
		handleDrop(event) {
			...
			if (!input || input.disabled || this.isDisabled() || !dropped || dropped.length === 0) return;
		...
		removeFileByIndex(event) {
			const input = this.$refs.input;
			if (!input?.files || input.disabled || this.isDisabled()) return;
			const indexRaw = event?.currentTarget?.dataset?.index;
			const index = Number.parseInt(indexRaw ?? "-1", 10);
			if (Number.isNaN(index) || index < 0 || index >= input.files.length) return;
		...
		applyDroppedFiles(input, droppedFiles) {
			...
			} else if (droppedFiles.length > 0) transfer.items.add(droppedFiles[0]);
			input.files = transfer.files;
		},
		...
		formatFileSize(size) {
			...
			return `${formatted >= 10 || power === 0 ? Math.round(formatted) : formatted.toFixed(1)} ${units[power]}`;
		},
		revokePreviews() {
...
//#endregion
//#region src/js/lib/components/rzInputOTP.js
function rzInputOTP() {
	return {
		value: "",
		length: 0,
		...
		*/
		focusSlot(event) {
			...
			if (!this.canFocusIndex(index)) return;
			this.clearSelection();
		...
		*/
		selectFilledSlots() {
			const filledIndexes = this.slots.map((slot, index) => ({
				slot,
				index
			})).filter(({ slot }) => slot.char !== "").map(({ index }) => index);
			...
			this.focusInput();
			const input = this.$refs.input;
			if (input) input.setSelectionRange(0, this.value.length);
			this.refreshSlots();
		},
		registerSlot() {
		...
		moveRight() {
			...
			if (!this.canFocusIndex(nextIndex)) return;
		...
		applyTextTransform(raw) {
			if (!raw) return "";
			if (this.textTransform === "to-lower") return raw.toLowerCase();
			if (this.textTransform === "to-upper") return raw.toUpperCase();
			return raw;
		},
		applyValueToInput() {
			const input = this.$refs.input;
		...
		isAcceptableInputChar(key) {
			...
			if (this.otpType === "alphanumeric") return /^[a-zA-Z0-9]$/.test(key);
			return /^[0-9]$/.test(key);
		},
		getNextFillIndex() {
		...
		canFocusIndex(index) {
			...
			if (this.value.charAt(normalizedIndex) !== "") return true;
		...
		updateSlotDom() {
			...
			(this.slotElements.length > 0 ? this.slotElements : fallbackRoot.querySelectorAll("[data-input-otp-slot=\"true\"]")).forEach((slotElement) => {
				...
				if (this.isInvalid) slotElement.setAttribute("aria-invalid", "true");
				else slotElement.removeAttribute("aria-invalid");
				const charElement = slotElement.querySelector("[data-input-otp-char=\"true\"]");
				if (charElement) charElement.textContent = state.char;
				const caretWrapper = slotElement.querySelector("[data-input-otp-caret=\"true\"]");
				if (caretWrapper) if (state.hasFakeCaret) caretWrapper.classList.remove("hidden");
				else caretWrapper.classList.add("hidden");
			});
		},
...
//#endregion
//#region src/js/lib/components/rzScrollArea.js
	...
	return {
		...
		*/
		updateCorner() {
			...
			if (!this.$refs.scrollbarX?.hidden && !this.$refs.scrollbarY?.hidden) {
			...
			} else this.$refs.corner.hidden = true;
		},
		/**
...
//#endregion
//#region src/js/lib/components/rzSlider.js
function rzSlider() {
	return {
		min: 0,
		max: 100,
		...
		init() {
			...
			if (assets.length > 0) rizzyRequire(assets, nonce);
		},
		...
		handlePointerMove(event) {
			if (this.disabled || !this.dragging || this.activeThumbIndex === null) return;
		...
		handleThumbKeyDown(event) {
			...
			if (event.key === "ArrowRight" || event.key === "ArrowUp") nextValue = current + this.step;
			else if (event.key === "ArrowLeft" || event.key === "ArrowDown") nextValue = current - this.step;
			else if (event.key === "PageUp") nextValue = current + largeStep;
			else if (event.key === "PageDown") nextValue = current - largeStep;
			else if (event.key === "Home") nextValue = this.min;
			else if (event.key === "End") nextValue = this.max;
			else return;
		...
		syncDom() {
			...
			if (this.trackEl) this.trackEl.dataset.orientation = this.orientation;
		...
		syncHiddenInputs() {
			this.inputEls.forEach((inputEl) => {
				...
				inputEl.value = `${this.values[index] ?? this.min}`;
			});
		},
		applyConstraints(values, changedIndex, requestedValue) {
			const count = values.length;
			if (count === 0) return [this.min];
			...
			} else constrained = [...constrained].sort((a, b) => a - b);
			const gap = this.minStepsBetweenThumbs;
			for (let index = 1; index < count; index += 1) {
				const minimum = constrained[index - 1] + gap;
				if (constrained[index] < minimum) constrained[index] = this.clampValue(this.snapValue(minimum));
			}
			for (let index = count - 2; index >= 0; index -= 1) {
				const maximum = constrained[index + 1] - gap;
				if (constrained[index] > maximum) constrained[index] = this.clampValue(this.snapValue(maximum));
			}
			return constrained;
		},
		snapValue(value) {
		...
		getPointerValue(event) {
			...
			if (denominator <= 0) return this.min;
			let ratio;
			if (this.orientation === "vertical") ratio = (rect.bottom - event.clientY) / denominator;
			else ratio = (event.clientX - rect.left) / denominator;
...
//# sourceMappingURL=advanced-input-runtime-CPxY73SZ.js.map