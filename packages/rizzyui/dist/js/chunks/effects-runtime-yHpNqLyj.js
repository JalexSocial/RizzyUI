...
//#endregion
//#region src/js/lib/components/rzConfetti.js
var DEFAULT_CUSTOM_EVENT = "rz:confetti";
...
function rzConfetti() {
	return {
		...
		setup() {
			...
			if (this.trigger === "load") this.fire();
		},
		bindTriggers() {
			...
			if (this.trigger === "visible") {
				this.intersectionObserver = new IntersectionObserver((entries) => this.handleVisibility(entries), { threshold: .25 });
		...
		async fireSequence(originOverride) {
			...
			for (let i = 0; i < iterations; i += 1) {
				...
				if (!instance) return;
				instance(options);
				if (i < iterations - 1 && this.burstIntervalMs > 0) await wait(this.burstIntervalMs);
			}
			this.hasFired = true;
		...
		buildOptions(originOverride, sequenceIndex = 0, totalIterations = 1) {
			const merged = {
				...this.getPresetOptions(sequenceIndex, totalIterations),
				...this.options
			};
			if (originOverride) merged.origin = originOverride;
			if (typeof this.options.originX === "number" || typeof this.options.originY === "number") merged.origin = {
				x: typeof this.options.originX === "number" ? this.options.originX : merged.origin?.x ?? .5,
				y: typeof this.options.originY === "number" ? this.options.originY : merged.origin?.y ?? .5
			};
		...
		getPresetOptions(sequenceIndex = 0, totalIterations = 1) {
			...
			return {
				...
				victory: {
					...
					origin: {
						x: .5,
						y: Math.max(.1, .65 - sequenceIndex / Math.max(1, totalIterations - 1) * .3)
					}
				},
				subtle: {
					...common,
			...
			}[kebabToCamel(this.preset)] || common;
		},
		resolveOrigin(event) {
			if (this.mode === "overlay") return null;
			if (this.mode === "scoped") return {
				x: .5,
				y: .5
			};
			if (this.launchFromPointer && event && typeof event.clientX === "number" && typeof event.clientY === "number") return {
		...
		ensureInstance() {
			...
			const confetti = window.confetti || confetti_module_default;
			if (typeof confetti !== "function" && typeof confetti?.create !== "function") {
				console.error("[rzConfetti] canvas-confetti was not found.");
				return null;
			}
			const moduleOrWindowConfetti = typeof confetti === "function" ? confetti : confetti_module_default;
			if (this.mode === "scoped") {
			...
			if (typeof confetti === "function") this.confettiFactory = confetti;
			else this.confettiFactory = moduleOrWindowConfetti;
			return this.confettiFactory;
		},
		...
		handleVisibility(entries) {
			if (!entries.some((entry) => entry.isIntersecting) || this.trigger !== "visible") return;
			this.fire();
		},
		canFire() {
		...
		destroy() {
			...
			if (this.$el && this.$el.rzConfetti) delete this.$el.rzConfetti;
		}
	};
}
//#endregion
...
//# sourceMappingURL=effects-runtime-yHpNqLyj.js.map