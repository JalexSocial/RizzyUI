...
public  class DefaultRzDropdownStyles : RzStylesBase.RzDropdownStylesBase
{
    ...
    /// <inheritdoc/>
    public override string MenuInnerContainer => $"border border-outline divide-y divide-{Theme.Light.Outline.TailwindClassName} rounded-borderRadius bg-{Theme.Light.Surface.TailwindClassName} ring-black dark:divide-outline"; // Simplified ring
}
...
public  class DefaultRzDropdownMenuItemStyles : RzStylesBase.RzDropdownMenuItemStylesBase
{
    ...
    /// <inheritdoc/>
    public override string CountDiv => $"inline-flex rounded-full border px-1.5 py-0.5 text-xs font-semibold border-outline"; // Added border color
}
