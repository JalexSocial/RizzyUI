...
public partial class RzQuickReferenceContainer : RzComponent
{
    ...
    [Parameter] public HeadingLevel MinimumHeadingLevel { get; set; } = HeadingLevel.H2;
    /// <summary> The highest heading level (e.g., H4) to include in the quick reference outline. Defaults to H4. </summary>
    [Parameter] public HeadingLevel MaximumHeadingLevel { get; set; } = HeadingLevel.H4;
    /// <summary> The child content of the container, which should include headings and potentially an <see cref="RzQuickReference"/> component. </summary>
    ...
    {
         _headingItemsSerialized = JsonSerializer.Serialize(_headingItems.Select(x => x.Id).ToList()); // Serialize the IDs
    ...
    protected override string? RootClass() =>
        TwMerge.Merge(AdditionalAttributes, Theme.RzQuickReferenceContainer.Container);
}
