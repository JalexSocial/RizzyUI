...
/// Automatically determines the 'for' attribute and infers the display name from the bound field if not explicitly set.
...
public partial class RzFieldLabel<TValue> : RzComponent
{
    ...
    /// <summary> Specifies the field the label is associated with. Used to determine the 'for' attribute and infer DisplayName. </summary>
    [Parameter] public Expression<Func<TValue>>? For { get; set; }
    /// <summary> Explicitly sets the text content of the label. If not set, it's inferred from the 'For' expression's display attributes or member name. </summary>
    ...
    private void SetEffectiveDisplayName()
    {
        ...
        if (For.Body is MemberExpression memberExpression)
        {
            ...
            _effectiveDisplayName = displayAttribute?.GetName() ?? memberExpression.Member.Name; // GetName handles resources
        }
        else
        {
    ...
    protected override string? RootClass() =>
        TwMerge.Merge(AdditionalAttributes, Theme.RzFieldLabel.Label);
}
