...
public partial class RzAvatar : RzComponent
{
    ...
    {
        get => _initials;
        set => _initials = value != null && value.Length > 2 ? value.Substring(0, 2).ToUpperInvariant() : (value ?? string.Empty).ToUpperInvariant();
    }
    ...
    [Parameter] public bool IndicatorVisible { get; set; } = false;
    /// <summary> Gets or sets the accent color of the status indicator. Defaults to Emerald. </summary>
    [Parameter] public AccentColor IndicatorColor { get; set; } = AccentColor.Emerald;
    /// <summary> Gets or sets a value indicating whether the avatar has a border (Currently not implemented in default styles). </summary>
    [Parameter] public bool Border { get; set; } = false; // Note: Border styling needs explicit addition if desired
    ...
    protected override string? RootClass() =>
        TwMerge.Merge(AdditionalAttributes, ContainerClass, Theme.RzAvatar.GetSizeCss(Size)); // Apply size to root
}
