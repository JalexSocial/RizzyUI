...
\**********************/
//#endregion
//#region src/js/runtime/validationRuntime.js
var import_aspnet_validation = (/* @__PURE__ */ __commonJSMin(((exports, module) => {
	(function webpackUniversalModuleDefinition(root, factory) {
		...
		return (() => {
			...
			(() => {
				__webpack_require__.r = (exports$2) => {
			...
			__webpack_require__.d(__webpack_exports__, {
				MvcValidationProviders: () => MvcValidationProviders,
			...
			var nullLogger = new (function() {
				function class_1() {
					this.warn = globalThis.console.warn;
				}
				class_1.prototype.log = function(_) {
					var _args = [];
					for (var _i = 1; _i < arguments.length; _i++) _args[_i - 1] = arguments[_i];
				};
				return class_1;
			}())();
			/**
			...
			*/
			function getRelativeFormElement(element, selector) {
				...
				if (dotLocation > -1) {
					...
					if (isValidatable(relativeElement)) return relativeElement;
				}
				...
				function MvcValidationProviders() {
					...
					this.required = function(value, element, params) {
						...
						if (elementType === "checkbox" || elementType === "radio") {
							...
							for (var _i = 0, allElementsOfThisName_1 = allElementsOfThisName; _i < allElementsOfThisName_1.length; _i++) {
								...
								if (element_1 instanceof HTMLInputElement && element_1.checked === true) return true;
							}
							if (elementType === "checkbox") {
								var checkboxHiddenInput = element.form.querySelector("input[name='".concat(element.name, "'][type='hidden']"));
								if (checkboxHiddenInput instanceof HTMLInputElement && checkboxHiddenInput.value === "false") return true;
							}
							return false;
						}
					...
					*/
					this.range = function(value, element, params) {
						...
						if (params.min) {
							if (val < parseFloat(params.min)) return false;
						}
						if (params.max) {
							if (val > parseFloat(params.max)) return false;
						}
						return true;
					};
					/**
					...
					*/
					this.phone = function(value, element, params) {
						if (!value) return true;
						if (/[\+\-\s][\-\s]/g.test(value)) return false;
					...
					*/
					this.remote = function(value, element, params) {
						...
						for (var _i = 0, fieldSelectors_1 = fieldSelectors; _i < fieldSelectors_1.length; _i++) {
							...
							if (!Boolean(fieldElement && fieldElement.value)) continue;
							if (fieldElement instanceof HTMLInputElement && (fieldElement.type === "checkbox" || fieldElement.type === "radio")) fields[fieldName] = fieldElement.checked ? fieldElement.value : "";
							else fields[fieldName] = fieldElement.value;
						}
						var url = params["url"];
						var encodedParams = [];
						...
						return new Promise(function(ok, reject) {
							...
							if (params.type && params.type.toLowerCase() === "post") {
								...
								for (var fieldName in fields) postData.append(fieldName, fields[fieldName]);
							...
							request.onload = function(e) {
								if (request.status >= 200 && request.status < 300) ok(JSON.parse(request.responseText));
								else reject({
			...
			var ValidationService = function() {
				function ValidationService(logger) {
					...
					this.validateForm = function(form, callback) {
						return __awaiter(_this, void 0, void 0, function() {
							...
							return __generator(this, function(_b) {
								switch (_b.label) {
									case 0:
										if (!(form instanceof HTMLFormElement)) throw new Error("validateForm() can only be called on <form> elements");
					...
					*/
					this.handleValidated = function(form, success, submitEvent) {
						if (!(form instanceof HTMLFormElement)) throw new Error("handleValidated() can only be called on <form> elements");
						if (success) {
							if (submitEvent) _this.submitValidForm(form, submitEvent);
						} else _this.focusFirstInvalid(form);
					};
					/**
					...
					*/
					this.submitValidForm = function(form, submitEvent) {
						if (!(form instanceof HTMLFormElement)) throw new Error("submitValidForm() can only be called on <form> elements");
						...
						if (form.dispatchEvent(newEvent)) {
							...
							try {
								...
								if (submitterInput) form.removeChild(submitterInput);
					...
					*/
					this.focusFirstInvalid = function(form) {
						if (!(form instanceof HTMLFormElement)) throw new Error("focusFirstInvalid() can only be called on <form> elements");
					...
					*/
					this.isValid = function(form, prevalidate, callback) {
						...
						if (!(form instanceof HTMLFormElement)) throw new Error("isValid() can only be called on <form> elements");
						if (prevalidate) _this.validateForm(form, callback);
						var formUID = _this.getElementUID(form);
						var formInputUIDs = _this.formInputs[formUID];
						return !((formInputUIDs === null || formInputUIDs === void 0 ? void 0 : formInputUIDs.some(function(uid) {
							return _this.summary[uid];
						})) === true);
					};
					/**
					...
					*/
					this.isFieldValid = function(field, prevalidate, callback) {
						if (prevalidate === void 0) prevalidate = true;
						if (prevalidate) _this.validateField(field, callback);
				...
				*/
				ValidationService.prototype.scanMessages = function(root, cb) {
					...
					for (var _i = 0, validationMessageElements_1 = validationMessageElements; _i < validationMessageElements_1.length; _i++) {
						...
						if (form instanceof HTMLFormElement) cb.call(this, form, span);
					}
					...
					if (containingForm) forms.push(containingForm);
				...
				ValidationService.prototype.pushValidationMessageSpan = function(form, span) {
					...
					if (spans.indexOf(span) < 0) spans.push(span);
					else this.logger.log("Validation element for '%s' is already tracked", name, span);
				};
				ValidationService.prototype.removeValidationMessageSpan = function(form, span) {
					...
					else this.logger.log("Validation element for '%s' was already removed", name, span);
				};
				/**
				...
				*/
				ValidationService.prototype.parseDirectives = function(attributes) {
					...
					for (var key in validationAtributes) _loop_1(key);
					return directives;
				};
				/**
				...
				*/
				ValidationService.prototype.getFormValidationTask = function(formUID) {
					...
					if (!formInputUIDs || formInputUIDs.length === 0) return Promise.resolve(true);
					var formValidators = [];
					for (var _i = 0, formInputUIDs_1 = formInputUIDs; _i < formInputUIDs_1.length; _i++) {
						...
						if (validator) formValidators.push(validator);
					}
				...
				*/
				ValidationService.prototype.trackFormInput = function(form, inputUID) {
					...
					if (formInputUIDs.indexOf(inputUID) === -1) {
						formInputUIDs.push(inputUID);
						if (this.options.addNoValidate) {
							this.logger.log("Setting novalidate on form", form);
							form.setAttribute("novalidate", "novalidate");
						} else this.logger.log("Not setting novalidate on form", form);
					} else this.logger.log("Form input for UID '%s' is already tracked", inputUID);
					if (this.formEvents[formUID]) return;
					var validationTask = null;
					var cb = function(e, callback) {
						if (validationTask) return validationTask;
						if (!_this.shouldValidate(e)) return Promise.resolve(true);
						validationTask = _this.getFormValidationTask(formUID);
						if (e) _this.preValidate(e);
								...
								return __generator(this, function(_a) {
									switch (_a.label) {
										case 0:
											...
											validationEvent = new CustomEvent("validation", { detail: { valid: success } });
				...
				ValidationService.prototype.reset = function(input) {
					if (this.isDisabled(input)) this.resetField(this.getElementUID(input));
					else this.scan(input);
				};
				...
				*/
				ValidationService.prototype.addInput = function(input) {
					...
					if (input.form) this.trackFormInput(input.form, uid);
					if (this.inputEvents[uid]) return;
							...
							return __generator(this, function(_a) {
								switch (_a.label) {
									case 0:
										...
										if (!validate) return [2, true];
										if (!input.dataset.valEvent && event && event.type === "input" && !input.classList.contains(this.ValidationInputCssClassName)) return [2, true];
					...
					var events = ((_a = input.dataset.valEvent) !== null && _a !== void 0 ? _a : defaultEvent).split(" ");
				...
				ValidationService.prototype.removeInput = function(input) {
					...
					if (input.form) this.untrackFormInput(input.form, uid);
				};
				/**
				...
				*/
				ValidationService.prototype.scanInputs = function(root, cb) {
					...
					if (isValidatable(root) && root.getAttribute("data-val") === "true") inputs.push(root);
					...
					for (var key in this.summary) {
						...
						if (matchingElement instanceof HTMLInputElement) {
							if (matchingElement.type === "checkbox" || matchingElement.type === "radio") {
								if (matchingElement.className === this.ValidationInputValidCssClassName) continue;
							}
						}
						if (renderedMessages.indexOf(this.summary[key]) > -1) continue;
				...
				*/
				ValidationService.prototype.renderSummary = function() {
					...
					if (shadow === this.renderedSummaryJSON) return;
					...
					for (var i = 0; i < summaryElements.length; i++) {
						...
						for (var j = 0; j < listElements.length; j++) listElements[j].remove();
						...
						} else this.swapClasses(e, this.ValidationSummaryValidCssClassName, this.ValidationSummaryCssClassName);
					}
				};
				/**
							...
							return __generator(this, function(_d) {
								switch (_d.label) {
									...
									case 2:
										...
										error = result;
										return [3, 5];
									case 3: return [4, result];
									case 4:
										resolution = _d.sent();
										if (typeof resolution === "boolean") valid = resolution;
										else {
											valid = false;
											error = resolution;
										}
				...
				*/
				ValidationService.prototype.swapClasses = function(element, addClass, removeClass) {
					if (addClass && !this.isDisabled(element) && !element.classList.contains(addClass)) element.classList.add(addClass);
					if (element.classList.contains(removeClass)) element.classList.remove(removeClass);
				};
				/**
				...
				*/
				ValidationService.prototype.bootstrap = function(options) {
					...
					var init = function() {
						_this.scan(root);
						if (_this.options.watch) _this.watch(root);
					};
				...
				*/
				ValidationService.prototype.scan = function(root) {
					root !== null && root !== void 0 || (root = this.options.root);
				...
				ValidationService.prototype.observed = function(mutation) {
					...
					if (mutation.type === "childList") {
						...
						for (var i = 0; i < mutation.removedNodes.length; i++) {
							...
							if (node instanceof HTMLElement) this.remove(node);
						}
					} else if (mutation.type === "attributes") {
						if (mutation.target instanceof HTMLElement) if (mutation.attributeName === "disabled") {
							...
							if (oldValue !== newValue) this.scan(mutation.target);
						}
					}
				};
				/**
...
var validation;
function ensureValidationRuntime() {
	if (!validation) {
		validation = new import_aspnet_validation.ValidationService();
		validation.bootstrap({ watch: true });
	}
	return validation;
}
//#endregion
export { ensureValidationRuntime };

//# sourceMappingURL=validationRuntime---GrPcL1.js.map