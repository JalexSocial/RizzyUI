import { r as flex } from "./bootstrap-CKfboHP5.js";
...
var Headers = { createTable: (table) => {
	table.getHeaderGroups = memo(() => [
		...
		return buildHeaderGroups$1(allColumns, [
			...leftColumns,
			...centerColumns,
			...rightColumns
		], table);
	...
	table.getLeftHeaderGroups = memo(() => [
		...
		return buildHeaderGroups$1(allColumns, (_left$map$filter2 = left == null ? void 0 : left.map((columnId) => leafColumns.find((d) => d.id === columnId)).filter(Boolean)) != null ? _left$map$filter2 : [], table, "left");
	...
	table.getRightHeaderGroups = memo(() => [
		...
		return buildHeaderGroups$1(allColumns, (_right$map$filter2 = right == null ? void 0 : right.map((columnId) => leafColumns.find((d) => d.id === columnId)).filter(Boolean)) != null ? _right$map$filter2 : [], table, "right");
...
//#endregion
//#region src/js/lib/components/rzDataTable.js
var selectionInputSelector = "input[data-rz-datatable-row-select][data-row-id]";
function readJsonConfig(rootEl) {
	const configId = rootEl?.dataset?.configId;
	if (!configId) throw new Error("[RizzyUI] rzDataTable requires data-config-id on the alpine host.");
	const scriptEl = document.getElementById(configId);
	if (!scriptEl) throw new Error(`[RizzyUI] rzDataTable config script '${configId}' was not found.`);
	return JSON.parse(scriptEl.textContent || "{}");
}
function getSegmentValue(source, segment) {
	if (source === null || source === void 0) return;
	if (typeof source !== "object") return;
	if (Object.prototype.hasOwnProperty.call(source, segment)) return source[segment];
	const camelCaseSegment = segment.length > 0 ? segment.charAt(0).toLowerCase() + segment.slice(1) : segment;
	if (Object.prototype.hasOwnProperty.call(source, camelCaseSegment)) return source[camelCaseSegment];
}
...
function normalizeColumns(columns) {
	return (columns || []).map((column) => {
		...
		if (Array.isArray(column.columns) && column.columns.length > 0) normalized.columns = normalizeColumns(column.columns);
		if (column.accessorKey) normalized.accessorFn = (row) => getByPath(row, column.accessorKey);
		if (column.cell === "rowSelection") normalized.cell = (cellContext) => {
		...
		else if (!column.cell) normalized.cell = (cellContext) => flex.text(cellContext.getValue?.() ?? "");
...
function buildRowModelGetters(pipeline) {
	...
	for (const key of [
		...
		pipeline?.sorted,
		pipeline?.paginated
	]) if (key && registry[key]) result[key] = registry[key]();
	return result;
}
...
function normalizePaginationSummary(component, pagination) {
	if (!pagination) return "Page changed.";
	...
	if (totalRows === 0) return `Page ${pageNumber} of ${pageCount}. No rows to display.`;
	return `Page ${pageNumber} of ${pageCount}. Showing rows ${startRow} to ${endRow} of ${totalRows}.`;
}
function normalizeSortSummary(sorting) {
	if (!sorting || sorting.length === 0) return "Sorting cleared.";
...
function normalizeFilterSummary(component, payload) {
	...
	const hasGlobalFilter = payload.globalFilter !== void 0 && payload.globalFilter !== null && String(payload.globalFilter) !== "";
	const filterCount = payload.columnFilters?.length || 0;
	if (!hasGlobalFilter && filterCount === 0) return `Filters cleared. ${totalRows} rows available.`;
...
function normalizeRowIdValue(value, rowIdPath, index) {
	if (value === null || value === void 0 || value === "") throw new Error(`[RizzyUI] rzDataTable could not resolve a stable non-empty row id for row at index ${index} using path '${rowIdPath}'.`);
	return String(value);
}
function validateRowIds(data, rowIdPath) {
	if (!rowIdPath || typeof rowIdPath !== "string") throw new Error("[RizzyUI] rzDataTable requires rowStructure.rowIdPath to resolve a stable row id for every row.");
	const seen = /* @__PURE__ */ new Map();
	for (let index = 0; index < data.length; index++) {
		const originalRow = data[index];
		const normalizedId = normalizeRowIdValue(getByPath(originalRow, rowIdPath), rowIdPath, index);
...
function buildFooterGroups(table) {
	return (table?.getFooterGroups?.() || []).map((group) => ({
		id: group.id,
		headers: [...group.headers]
	}));
}
...
function buildPaginationItems(pageIndex, pageCount) {
	...
	if (pageCount <= 7) return Array.from({ length: pageCount }, (_, index) => ({
		kind: "page",
		id: `page-${index}`,
		pageIndex: index,
	...
	const normalizedPages = [...new Set([
		...
		pageIndex,
		pageIndex + 1
	])].filter((x) => x >= 0 && x < pageCount).sort((a, b) => a - b);
	const items = [];
	let previousPage = null;
	for (const page of normalizedPages) {
		if (previousPage !== null && page - previousPage > 1) items.push({
			kind: "ellipsis",
...
function resolveSortDirection(component, header) {
	...
	const entry = (component.table?.getState?.()?.sorting || []).find((x) => x?.id === columnId);
...
function createSortApi(component) {
	return {
		...
		nextLabel: (header) => {
			if (!header?.column?.getCanSort?.()) return "Sorting unavailable";
			const value = resolveSortDirection(component, header);
			const enableSortingRemoval = component.table?.options?.enableSortingRemoval !== false;
			if (value === "asc") return "Sort descending";
			if (value === "desc") return enableSortingRemoval ? "Clear sort" : "Sort ascending";
...
function createSelectionApi(component) {
	return {
		...
		setRowSelected: (row, value) => {
			if (row?.getCanSelect?.() === false) return;
...
function createPaginationApi(component) {
	return {
		...
		items: [],
		previousPage() {
			if (!component.table || !component.table.getCanPreviousPage()) return;
			component.table.previousPage();
		},
		nextPage() {
			if (!component.table || !component.table.getCanNextPage()) return;
...
function rzDataTable() {
	return {
		...
		init() {
			...
			table = createTable({
				...
				getRowId: (originalRow, index) => {
					return normalizeRowIdValue(getByPath(originalRow, rowIdPath), rowIdPath, index);
				},
			...
			this.dispatchEvent("rz:datatable:ready", { componentId });
		},
		destroy() {
		...
		bindGeneratedSelectionCells(componentId) {
			if (!this.$el?.addEventListener || this._boundSelectionChange) return;
			this._boundSelectionChange = (event) => {
				const target = event.target;
				if (!target?.matches?.(selectionInputSelector)) return;
		...
		refreshDerivedState() {
			...
			if (this.filter) this.filter.globalFilter = this.table?.getState?.().globalFilter ?? "";
			if (this.columns) {
		...
		refreshPaginationState() {
			...
			const endRow = totalRows === 0 || visibleRows === 0 ? 0 : startRow + visibleRows - 1;
		...
		dispatchStateEvents(componentId) {
			...
			const payload = normalizeStatePayload(this.table.getState(), componentId, this.table);
		...
		setAnnouncement(reason, message) {
			if (!message || this.announcement.message === message) return;
			this.announcement = {
				message,
				politeness: "polite",
				reason
...
//# sourceMappingURL=table-runtime-DuZe8pjY.js.map