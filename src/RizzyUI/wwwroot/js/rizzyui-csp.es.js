...
function onMutate(mutations) {
  ...
  for (let i2 = 0; i2 < mutations.length; i2++) {
    ...
    if (mutations[i2].type === "attributes") {
      ...
      let name2 = mutations[i2].attributeName;
...
var mergeProxyTrap = {
  ...
  set({ objects }, name2, value, thisProxy) {
  ...
  return {
    before(directive2) {
      if (!directiveHandlers[directive2]) {
        console.warn(String.raw`Cannot find directive \`${directive2}\`. \`${name2}\` will use the default order of execution`);
        return;
      }
...
function directives(el, attributes, originalAttributeOverride) {
  ...
  if (el._x_virtualDirectives) {
    let vAttributes = Object.entries(el._x_virtualDirectives).map(([name2, value]) => ({ name: name2, value }));
...
var startingWith = (subject, replacement) => ({ name: name2, value }) => {
...
function bind(el, name2, value, modifiers = []) {
  if (!el._x_bindings)
  ...
  switch (name2) {
    ...
    case "selected":
    case "checked":
      bindAttributeAndProperty(el, name2, value);
      break;
    default:
...
function bindAttributeAndProperty(el, name2, value) {
  bindAttribute(el, name2, value);
  setPropertyIfChanged(el, name2, value);
}
function bindAttribute(el, name2, value) {
  if ([null, void 0, false].includes(value) && attributeShouldntBePreservedIfFalsy(name2)) {
...
function getAttributeBinding(el, name2, fallback) {
  let attr = el.getAttribute(name2);
  if (attr === null)
    return typeof fallback === "function" ? fallback() : fallback;
  if (attr === "")
    return true;
  if (isBooleanAttr(name2)) {
    return !![name2, "true"].includes(attr);
  }
  return attr;
}
...
function applyBindingsObject(el, obj, original) {
  ...
  while (cleanupRunners.length)
    cleanupRunners.pop()();
  let attributes = Object.entries(obj).map(([name2, value]) => ({ name: name2, value }));
  ...
  parseObjectLiteral() {
    ...
    while (!this.check("PUNCTUATION", "}") && !this.isAtEnd()) {
      ...
      if (this.match("STRING")) {
        ...
        const name2 = this.previous().value;
    ...
    switch (node.type) {
      ...
      case "CallExpression":
        ...
        if (node.callee.type === "MemberExpression") {
          ...
          if (node.callee.type === "Identifier") {
            const name2 = node.callee.name;
            let func;
            if (name2 in scope2) {
              func = scope2[name2];
            } else {
              throw new Error(`Undefined variable: ${name2}`);
            }
            if (typeof func !== "function") {
              throw new Error("Value is not a function");
            }
      ...
      case "UpdateExpression":
        if (node.argument.type === "Identifier") {
          const name2 = node.argument.name;
          if (!(name2 in scope2)) {
            throw new Error(`Undefined variable: ${name2}`);
          }
          const oldValue = scope2[name2];
          if (node.operator === "++") {
            scope2[name2] = oldValue + 1;
          } else if (node.operator === "--") {
            scope2[name2] = oldValue - 1;
          }
          return node.prefix ? scope2[name2] : oldValue;
...
magic("id", (el, { cleanup: cleanup2 }) => (name2, key = null) => {
  let cacheKey = `${name2}${key ? `-${key}` : ""}`;
  return cacheIdByNameOnElement(el, cacheKey, cleanup2, () => {
    let root = closestIdRoot(el, name2);
    let id = root ? root._x_ids[name2] : findAndIncrementId(name2);
...
function on(el, event2, modifiers, callback) {
  ...
  if (modifiers.includes("debounce")) {
    ...
    let wait2 = isNumeric(nextModifier.split("ms")[0]) ? Number(nextModifier.split("ms")[0]) : 250;
    handler4 = debounce(handler4, wait2);
  }
  if (modifiers.includes("throttle")) {
    let nextModifier = modifiers[modifiers.indexOf("throttle") + 1] || "invalid-wait";
    let wait2 = isNumeric(nextModifier.split("ms")[0]) ? Number(nextModifier.split("ms")[0]) : 250;
    handler4 = throttle(handler4, wait2);
  }
...
function async_alpine_default(Alpine2) {
  ...
  Alpine2.asyncData = (name2, download2 = false) => {
    data2[name2] = {
      loaded: false,
      download: download2
    };
  };
  Alpine2.asyncUrl = (name2, url) => {
    if (!name2 || !url || data2[name2]) return;
    data2[name2] = {
      loaded: false,
      download: () => import(
        /* @vite-ignore */
  ...
  const handler4 = async (el) => {
    Alpine2.skipDuringClone(async () => {
      ...
      el._x_async = "await";
      const { name: name2, strategy } = elementPrep(el);
  ...
  function elementPrep(el) {
    const name2 = parseName(el.getAttribute(Alpine2.prefixed("data")));
    ...
    if (urlAttributeValue) {
      Alpine2.asyncUrl(name2, urlAttributeValue);
    }
    return {
      name: name2,
      strategy
  ...
  async function download(name2) {
    if (name2.startsWith("_x_async_")) return;
    handleAlias(name2);
    if (!data2[name2] || data2[name2].loaded) return;
    const module = await getModule(name2);
    Alpine2.data(name2, module);
    data2[name2].loaded = true;
  }
  async function getModule(name2) {
    if (!data2[name2]) return;
    const module = await data2[name2].download(name2);
    if (typeof module === "function") return module;
    let whichExport = module[name2] || module.default || Object.values(module)[0] || false;
    return whichExport;
  }
  ...
  function handleAlias(name2) {
    if (!alias || data2[name2]) return;
    if (typeof alias === "function") {
      Alpine2.asyncData(name2, alias);
      return;
    }
    Alpine2.asyncUrl(name2, alias.replaceAll("[name]", name2));
  }
      ...
      return (
        ...
        (() => {
          ...
          var validatableSelector = function(selector) {
            return validatableElementTypes.map(function(t2) {
              return "".concat(t2).concat(selector || "");
            }).join(",");
          };
            ...
            /* @__PURE__ */ (function() {
              function MvcValidationProviders2() {
                ...
                this.stringLength = function(value, element, params) {
                  ...
                  if (params.min) {
                    var min2 = parseInt(params.min);
                    if (value.length < min2) {
                      return false;
                    }
                  }
                  if (params.max) {
                    var max2 = parseInt(params.max);
                ...
                this.range = function(value, element, params) {
                  ...
                  if (params.min) {
                    var min2 = parseFloat(params.min);
                    if (val < min2) {
                      return false;
                    }
                  }
                  if (params.max) {
                    var max2 = parseFloat(params.max);
                    if (val > max2) {
                      return false;
                    }
                  }
                ...
                this.regex = function(value, element, params) {
                  ...
                  var r2 = new RegExp(params.pattern);
                  return r2.test(value);
                };
                this.email = function(value, element, params) {
                  if (!value) {
                    return true;
                  }
                  var r2 = /^([^\x00-\x20\x22\x28\x29\x2c\x2e\x3a-\x3c\x3e\x40\x5b-\x5d\x7f-\xff]+|\x22([^\x0d\x22\x5c\x80-\xff]|\x5c[\x00-\x7f])*\x22)(\x2e([^\x00-\x20\x22\x28\x29\x2c\x2e\x3a-\x3c\x3e\x40\x5b-\x5d\x7f-\xff]+|\x22([^\x0d\x22\x5c\x80-\xff]|\x5c[\x00-\x7f])*\x22))*\x40([^\x00-\x20\x22\x28\x29\x2c\x2e\x3a-\x3c\x3e\x40\x5b-\x5d\x7f-\xff]+|\x5b([^\x0d\x5b-\x5d\x80-\xff]|\x5c[\x00-\x7f])*\x5d)(\x2e([^\x00-\x20\x22\x28\x29\x2c\x2e\x3a-\x3c\x3e\x40\x5b-\x5d\x7f-\xff]+|\x5b([^\x0d\x5b-\x5d\x80-\xff]|\x5c[\x00-\x7f])*\x5d))*(\.\w{2,})+$/;
                  return r2.test(value);
                };
                this.creditcard = function(value, element, params) {
                  ...
                  var nCheck = 0, nDigit = 0, bEven = false, n2, cDigit;
                  value = value.replace(/\D/g, "");
                  if (value.length < 13 || value.length > 19) {
                    return false;
                  }
                  for (n2 = value.length - 1; n2 >= 0; n2--) {
                    cDigit = value.charAt(n2);
            ...
            (function() {
              function ValidationService2(logger) {
                ...
                this.focusFirstInvalid = function(form) {
                  ...
                  var invalidFormInputUID = formInputUIDs === null || formInputUIDs === void 0 ? void 0 : formInputUIDs.find(function(uid2) {
                    return _this.summary[uid2];
                  });
                ...
                this.isValid = function(form, prevalidate, callback) {
                  ...
                  var formUID = _this.getElementUID(form);
                  var formInputUIDs = _this.formInputs[formUID];
                  var formIsInvalid = (formInputUIDs === null || formInputUIDs === void 0 ? void 0 : formInputUIDs.some(function(uid2) {
                    return _this.summary[uid2];
                  })) === true;
              ...
              ValidationService2.prototype.parseDirectives = function(attributes) {
                ...
                for (var i2 = 0; i2 < attributes.length; i2++) {
                  var a2 = attributes[i2];
                  if (a2.name.indexOf("data-val-") === 0) {
                    var key = a2.name.substr(cut);
                    validationAtributes[key] = a2.value;
                  }
                }
                var _loop_1 = function(key2) {
                  if (key2.indexOf("-") === -1) {
                    ...
                    for (var i3 = 0; i3 < parameters.length; i3++) {
                      var pvalue = validationAtributes[parameters[i3]];
                      var pkey = parameters[i3].substr(pcut);
                      directive2.params[pkey] = pvalue;
                    }
                    directives2[key2] = directive2;
                  }
                };
              ...
              ValidationService2.prototype.getElementUID = function(node) {
                var x = this.elementUIDs.filter(function(e2) {
                  return e2.node === node;
                })[0];
                if (x) {
                  return x.uid;
              ...
              ValidationService2.prototype.getFormValidationTask = function(formUID) {
                ...
                return Promise.all(tasks).then(function(result) {
                  return result.every(function(e2) {
                    return e2;
                  });
                });
              };
              ...
              ValidationService2.prototype.shouldValidate = function(e2) {
                return !(e2 && e2["submitter"] && e2["submitter"]["formNoValidate"]);
              };
              ValidationService2.prototype.trackFormInput = function(form, inputUID) {
                ...
                var add2 = formInputUIDs.indexOf(inputUID) === -1;
                if (add2) {
                ...
                var cb = function(e2, callback) {
                  if (validationTask) {
                    return validationTask;
                  }
                  if (!_this.shouldValidate(e2)) {
                    return Promise.resolve(true);
                  }
                  validationTask = _this.getFormValidationTask(formUID);
                  if (e2) {
                    _this.preValidate(e2);
                  }
                    ...
                    return __awaiter(_this, void 0, void 0, function() {
                      ...
                      return __generator(this, function(_a2) {
                        switch (_a2.label) {
                          ...
                          case 1:
                            _a2.sent();
                            this.handleValidated(form, success, e2);
                            return [2, success];
                        }
                      });
                    });
                  }).catch(function(error2) {
                    _this.logger.log("Validation error", error2);
                    return false;
                  }).finally(function() {
                    validationTask = null;
                  });
              ...
              ValidationService2.prototype.resetField = function(inputUID) {
                ...
                if (spans) {
                  for (var i2 = 0; i2 < spans.length; i2++) {
                    spans[i2].innerHTML = "";
                    this.swapClasses(spans[i2], "", this.ValidationMessageCssClassName);
                    this.swapClasses(spans[i2], "", this.ValidationMessageValidCssClassName);
                  }
                }
              ...
              ValidationService2.prototype.addInput = function(input) {
                var _this = this;
                var _a;
                var uid2 = this.getElementUID(input);
                var directives2 = this.parseDirectives(input.attributes);
                this.validators[uid2] = this.createValidator(input, directives2);
                if (input.form) {
                  this.trackFormInput(input.form, uid2);
                }
                if (this.inputEvents[uid2]) {
                  return;
                }
                var cb = function(event2, callback) {
                  return __awaiter(_this, void 0, void 0, function() {
                    var validate, success, error_1;
                    return __generator(this, function(_a2) {
                      switch (_a2.label) {
                        case 0:
                          validate = this.validators[uid2];
                          if (!validate)
                            return [2, true];
                          if (!input.dataset.valEvent && event2 && event2.type === "input" && !input.classList.contains(this.ValidationInputCssClassName)) {
                            return [2, true];
                          }
                          this.logger.log("Validating", { event: event2 });
                          _a2.label = 1;
                        case 1:
                          _a2.trys.push([1, 3, , 4]);
                          return [4, validate()];
                ...
                cb.debounced = function(event2, callback) {
                ...
                this.inputEvents[uid2] = cb;
              };
              ValidationService2.prototype.removeInput = function(input) {
                var uid2 = this.getElementUID(input);
                var cb = this.inputEvents[uid2];
                if (cb === null || cb === void 0 ? void 0 : cb.remove) {
                  cb.remove();
                  delete cb.remove;
                }
                delete this.summary[uid2];
                delete this.inputEvents[uid2];
                delete this.validators[uid2];
                if (input.form) {
                  this.untrackFormInput(input.form, uid2);
                }
              };
              ...
              ValidationService2.prototype.renderSummary = function() {
                ...
                for (var i2 = 0; i2 < summaryElements.length; i2++) {
                  var e2 = summaryElements[i2];
                  var listElements = e2.querySelectorAll("ul");
                  for (var j = 0; j < listElements.length; j++) {
                    listElements[j].remove();
                  }
                  if (ul && ul.hasChildNodes()) {
                    this.swapClasses(e2, this.ValidationSummaryCssClassName, this.ValidationSummaryValidCssClassName);
                    e2.appendChild(ul.cloneNode(true));
                  } else {
                    this.swapClasses(e2, this.ValidationSummaryValidCssClassName, this.ValidationSummaryCssClassName);
                  }
                }
              };
              ValidationService2.prototype.addError = function(input, message) {
                ...
                if (spans) {
                  for (var i2 = 0; i2 < spans.length; i2++) {
                    spans[i2];
                    spans[i2].innerHTML = message;
                    this.swapClasses(spans[i2], this.ValidationMessageCssClassName, this.ValidationMessageValidCssClassName);
                  }
                }
                ...
                if (input.form) {
                  ...
                  for (var i2 = 0; i2 < inputs.length; i2++) {
                    this.swapClasses(inputs[i2], this.ValidationInputCssClassName, this.ValidationInputValidCssClassName);
                    var uid2 = this.getElementUID(inputs[i2]);
                    this.summary[uid2] = message;
                  }
                }
                this.renderSummary();
              };
              ValidationService2.prototype.removeError = function(input) {
                ...
                if (spans) {
                  for (var i2 = 0; i2 < spans.length; i2++) {
                    spans[i2].innerHTML = "";
                    this.swapClasses(spans[i2], this.ValidationMessageValidCssClassName, this.ValidationMessageCssClassName);
                  }
                }
                ...
                if (input.form) {
                  var inputs = input.form.querySelectorAll(validatableSelector('[name="'.concat(input.name, '"]')));
                  for (var i2 = 0; i2 < inputs.length; i2++) {
                    this.swapClasses(inputs[i2], this.ValidationInputValidCssClassName, this.ValidationInputCssClassName);
                    var uid2 = this.getElementUID(inputs[i2]);
                    delete this.summary[uid2];
                  }
                }
              ...
              ValidationService2.prototype.createValidator = function(input, directives2) {
                var _this = this;
                return function() {
                  return __awaiter(_this, void 0, void 0, function() {
                    var _a, _b, _c, _i, key, directive2, provider, result, valid, error2, resolution;
                    return __generator(this, function(_d) {
                      switch (_d.label) {
                        ...
                        case 1:
                          ...
                          result = provider(input.value, input, directive2.params);
                          valid = false;
                          error2 = directive2.error;
              ...
              ValidationService2.prototype.observed = function(mutation) {
                ...
                if (mutation.type === "childList") {
                  for (var i2 = 0; i2 < mutation.addedNodes.length; i2++) {
                    var node = mutation.addedNodes[i2];
                  ...
                  for (var i2 = 0; i2 < mutation.removedNodes.length; i2++) {
                    var node = mutation.removedNodes[i2];
...
function registerRzCalendar(Alpine2, require2) {
  Alpine2.data("rzCalendar", () => ({
    ...
    initCalendar(configId) {
      ...
      const eventHandlers = {
        onClickDate: (self2, e2) => {
          this.dispatchCalendarEvent("click-day", {
            event: e2,
            dates: self2.context.selectedDates
          });
        },
        onClickWeekNumber: (self2, number, year, dateEls, e2) => {
        ...
        onClickMonth: (self2, e2) => {
          this.dispatchCalendarEvent("click-month", {
            event: e2,
            month: self2.context.selectedMonth
          });
        },
        onClickYear: (self2, e2) => {
          this.dispatchCalendarEvent("click-year", {
            event: e2,
            year: self2.context.selectedYear
          });
        },
        onClickArrow: (self2, e2) => {
        ...
        onChangeTime: (self2, e2, isError) => {
          this.dispatchCalendarEvent("change-time", {
            event: e2,
            time: self2.context.selectedTime,
            hours: self2.context.selectedHours,
            minutes: self2.context.selectedMinutes,
            keeping: self2.context.selectedKeeping,
            isError
          });
        }
      };
...
function registerRzClipboard(Alpine2) {
  Alpine2.data("rzClipboard", () => ({
    value: null,
    targetSelector: null,
    preferValue: false,
    ...
    getTextToCopy() {
      ...
      if (this.targetSelector) {
        ...
        if (target) {
          return target.value !== void 0 ? target.value : target.textContent;
        }
      }
      return this.value;
    },
    ...
    onSuccess(text) {
      this.copied = true;
      this.$dispatch("rz:copy", { id: this.$el.dataset.alpineRoot, text });
    ...
    dispatchFailed(reason, error2 = null) {
...
function registerRzBackToTop(Alpine2) {
  Alpine2.data("rzBackToTop", () => ({
    visible: false,
    threshold: 300,
    _rafPending: false,
    _onScroll: null,
...
(function main(global, module, isWorker, workerSize) {
  ...
  var getWorker = /* @__PURE__ */ (function() {
    ...
    function decorate(worker2) {
      ...
      worker2.fire = function fireWorker(options, size2, done2) {
        if (prom) {
    ...
    return function() {
      ...
      if (!isWorker && canUseWorker) {
        ...
        try {
          ...
          typeof console !== "undefined" && typeof console.warn === "function" ? console.warn("🎊 Could not load worker", e2) : null;
          return null;
        }
        decorate(worker);
      }
  ...
  function prop(options, name2, transform) {
    return convert(
  ...
  function animate(canvas, fettis, resizer, size2, done2) {
  ...
  function confettiCannon(canvas, globalOpts) {
    ...
    var animationObj;
    function fireLocal(options, size2, done2) {
      ...
      animationObj = animate(canvas, fettis, resizer, size2, done2);
    ...
    function fire(options) {
      ...
      if (worker) {
        return worker.fire(options, size2, done2);
      }
      return fireLocal(options, size2, done2);
    }
    fire.reset = function() {
      if (worker) {
        worker.reset();
...
function kebabToCamel(value) {
  return (value || "").replace(/-([a-z])/g, (_, c2) => c2.toUpperCase());
}
function wait(ms) {
  return new Promise((resolve) => window.setTimeout(resolve, ms));
}
function registerRzConfetti(Alpine2, require2) {
  Alpine2.data("rzConfetti", () => ({
    ...
    init() {
      ...
      if (assets.length > 0 && typeof require2 === "function") {
        require2(assets, {
          success: () => this.setup(),
          error: (error2) => {
            console.error("[rzConfetti] Failed to load confetti assets.", error2);
            this.setup();
          }
        }, nonce);
        return;
    ...
    bindTriggers() {
      if (this.trigger === "custom-event") {
        this.customEventHandler = (event2) => this.handleCustomEvent(event2);
    ...
    fire(event2) {
      const originOverride = this.resolveOrigin(event2);
    ...
    async fireSequence(originOverride) {
      ...
      for (let i2 = 0; i2 < iterations; i2 += 1) {
        const options = this.buildOptions(originOverride, i2, iterations);
        const instance = this.ensureInstance();
        if (!instance) return;
        instance(options);
        if (i2 < iterations - 1 && this.burstIntervalMs > 0) {
    ...
    resolveOrigin(event2) {
      ...
      if (this.launchFromPointer && event2 && typeof event2.clientX === "number" && typeof event2.clientY === "number") {
        return {
          x: event2.clientX / window.innerWidth,
          y: event2.clientY / window.innerHeight
        };
      }
    ...
    handleCustomEvent(event2) {
      if (this.trigger !== "custom-event") return;
      this.fireSequence(this.resolveOrigin(event2?.detail?.event));
    },
...
function registerRzColorPickerProvider(Alpine2, require2) {
...
function registerRzHighlighter(Alpine2, require2) {
  Alpine2.data("rzHighlighter", () => ({
    annotation: null,
    resizeObserver: null,
    intersectionObserver: null,
    rafId: 0,
...
function registerRzNumberTicker(Alpine2) {
  Alpine2.data("rzNumberTicker", () => ({
    targetValue: 0,
    startValue: 0,
    currentValue: 0,
    destinationValue: 0,
...
function registerRzPopover(Alpine2) {
  Alpine2.data("rzPopover", () => ({
    ...
    handleWindowKeydown(event2) {
      if (event2.key !== "Escape" || !this.open) {
        return;
      }
      this.open = false;
      this.$nextTick(() => this.restoreTriggerFocus());
...
function registerRzSlider(Alpine2) {
  Alpine2.data("rzSlider", () => ({
    min: 0,
    max: 100,
    step: 1,
    ...
    handlePointerDown(event2) {
      ...
      const thumbIndex = this.parseThumbIndex(event2.currentTarget);
      if (thumbIndex < 0) {
        return;
      }
    ...
    applyConstraints(values, changedIndex, requestedValue) {
      ...
      if (changedIndex >= 0 && changedIndex < count) {
        ...
        constrained = [...constrained].sort((a2, b) => a2 - b);
      }
    ...
    getPointerValue(event2) {
      ...
      let ratio;
      if (this.orientation === "vertical") {
        ratio = (rect.bottom - event2.clientY) / denominator;
      } else {
        ratio = (event2.clientX - rect.left) / denominator;
      }
...
function registerRzShineBorder(Alpine2) {
  Alpine2.data("rzShineBorder", () => ({
    computedStyle: "",
    init() {
      const dataset = this.$el.dataset;
...
function registerRzTooltip(Alpine2) {
  Alpine2.data("rzTooltip", () => ({
     ...
     */
    getBooleanDataset(name2, fallbackValue) {
      const value = this.$el.dataset[name2];
     ...
     */
    getNumberDataset(name2, fallbackValue) {
      const value = Number(this.$el.dataset[name2]);
     ...
     */
    getNullableNumberDataset(name2, fallbackValue) {
      const raw2 = this.$el.dataset[name2];
  ...
  Alpine2.data("rzCommand", () => ({
    ...
    normalizeItem(item) {
      ...
      return {
        ...
        id: this.resolveStableItemId(item),
        keywords,
        _searchText: `${name2} ${keywords.join(" ")}`.trim().toLowerCase(),
     ...
     * @returns {void}
     */
    registerGroupTemplate(name2, templateContent, headingId) {
      if (!name2 || !templateContent || this.groupTemplates.has(name2)) {
        return;
      }
      this.groupTemplates.set(name2, {
        headingId,
        templateContent
      });
    },
...
function registerRzCommandItem(Alpine2) {
  Alpine2.data("rzCommandItem", () => ({
    ...
    resolveItemId(value, name2) {
      ...
      return `item-${this.hashString(name2 || this.$el.textContent.trim())}`;
    },
    /**
    ...
    init() {
      ...
      const name2 = this.$el.dataset.name || this.$el.dataset.value || this.$el.textContent.trim();
      this.itemData = {
        id: this.resolveItemId(value, name2),
        value,
        name: name2,
...
function registerRzChart(Alpine2, require2) {
  Alpine2.data("rzChart", () => ({
    chartInstance: null,
    ...
    init() {
      ...
      if (assetsToLoad.length > 0 && typeof require2 === "function") {
        require2(assetsToLoad, {
          ...
          error: (error2) => console.error("[rzChart] Failed to load Chart.js assets.", error2)
        }, nonce);
        return;
      }
    ...
    tryInitializeChart(configScriptId) {
      ...
      try {
        ...
        console.error("[rzChart] Failed to parse JSON configuration.", error2);
        return;
      }
    ...
    resolveColorValues(config) {
      const walk2 = (value) => {
        if (Array.isArray(value)) {
          return value.map((item) => walk2(item));
        }
...
function registerRzTypingAnimation(Alpine2) {
  Alpine2.data("rzTypingAnimation", () => ({
    config: null,
    words: [],
    segmentedWords: [],
    displayText: "",
...
function registerComponents(Alpine2) {
  ...
  registerRzClipboard(Alpine2);
  registerRzBackToTop(Alpine2);
  registerRzCombobox(Alpine2, rizzyRequire);
  registerRzConfetti(Alpine2, rizzyRequire);
  ...
  registerRzHighlighter(Alpine2, rizzyRequire);
  ...
  registerRzNumberTicker(Alpine2);
  ...
  registerRzSlider(Alpine2);
  registerRzSheet(Alpine2);
  registerRzShineBorder(Alpine2);
  ...
  registerRzChart(Alpine2, rizzyRequire);
  registerRzTypingAnimation(Alpine2);
}
...
function setAsyncLoader(name2, path) {
  const Alpine2 = globalThis.Alpine;
  if (!(Alpine2 && typeof Alpine2.asyncData === "function")) {
    console.error(
      `[RizzyUI] Could not register async component '${name2}'. AsyncAlpine not available.`
    );
    return false;
  }
  Alpine2.asyncData(
    ...
    () => onceImport(path).catch((error2) => {
      console.error(
        `[RizzyUI] Failed to load Alpine module '${name2}' from '${path}'.`,
        error2
      );
      return () => ({
        _error: true,
        _errorMessage: `Module '${name2}' failed to load.`
      });
    })
  );
  return true;
}
function registerAsyncComponent(name2, path) {
  if (!name2 || !path) {
    console.error("[RizzyUI] registerAsyncComponent requires both name and path.");
    return;
  }
  const prev = _registered.get(name2);
  if (prev && prev.path !== path) {
    console.warn(
      `[RizzyUI] Re-registering '${name2}' with a different path.
  Previous: ${prev.path}
  New:      ${path}`
    );
  }
  ...
  if (Alpine2 && Alpine2.version) {
    ...
    if (!alreadySet) {
      const ok = setAsyncLoader(name2, path);
      _registered.set(name2, { path, loaderSet: ok });
    }
    return;
  }
  _registered.set(name2, { path, loaderSet: false });
...
function bootstrapRizzyUI(Alpine2) {
  ...
  const validation = new aspnetValidationExports.ValidationService();
